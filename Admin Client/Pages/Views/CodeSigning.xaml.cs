...
{
    public sealed partial class CodeSigning : Page
    {
        ...
        private async Task LoadEntriesAsync()
        {
            try
            {
                ...
                var query = _certificateService.GetCertificateEntries(db, "code", "Name", typeof(EKU))
        ...
        private async void ExportButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var allEntries = await _certificateService.GetCertificateEntries(db, "Code Signing", "Name", typeof(EKU))
                    .Where(e => e.RequestDisposition == "20" && e.CertificateExpirationDate >= DateTime.Now)
                ...
                savePicker.SuggestedFileName = $"CodeSigningCertificates_{DateTime.Now:yyyyMMdd}";
                ...
                if (file != null)
                {
                    ...
                    sb.AppendLine("RequestId,CAConfig,Subject,Template,ExpirationDate,EKU,Owner,Notes");
                    foreach (var entry in allEntries)
                    {
                        sb.AppendLine($"\"{entry.RequestId}\",\"{Escape(entry.CAConfig)}\",\"{Escape(entry.RequestCommonName)}\",\"{Escape(entry.CertificateTemplate)}\",\"{entry.CertificateExpirationDate}\",\"{Escape(entry.EKUNames)}\",\"{Escape(entry.Owner)}\",\"{Escape(entry.Notes)}\"");
                    }
